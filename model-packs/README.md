...
A model-pack is the authored bundle for a modeled system. It supplies the **D (Domain)** pillar of the D.S.A. Framework while also carrying the pack-owned state conventions, adapters, prompts, docs, and optional UI. It defines:
...
```bash
mkdir -p model-packs/{org}/{subject-level}
```
...
python ../reference-implementations/yaml-to-json-converter.py \
  model-packs/{org}/{subject-level}/domain-physics.yaml \
...
python ../reference-implementations/system-log-validator.py \
  --commit model-packs/{org}/{subject-level}/domain-physics.json \
