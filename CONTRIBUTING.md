# Contributing to the Lumina Framework

Welcome, and thank you for your interest in the Lumina Framework.

Lumina is not a chatbot; it is a **domain-grounded, measurement-centric neuro-symbolic systems framework**. Our core philosophy is **Accountability by Design**. Every contribution must uphold the integrity of the **D.S.A. (Domain, State, Actor)** prompt contracts and the cryptographic accountability of the **System Logs**.

Whether you are optimizing the core orchestration engine or authoring a new model-pack, please read this guide carefully to understand our strict architectural boundaries.

---

## Architectural Boundaries

Before contributing, you must understand the firewall between the core engine and the model-packs.

### 1. The Core Engine (`src/lumina/`)
...
All modeled-system-specific behavior lives here. If you are adding a new use case, you are authoring a model-pack.

* You must define the rules of reality in a `domain-physics.json` file.
* You must provide a complete `runtime-config.yaml` to map actions to your specific Python tool adapters.
* **Fractal Authority:** Your model-pack must clearly define its governance levels (Macro → Meso → Micro → Target).
...
### Adding or Modifying a Model-Pack

We welcome new reference implementations for different systems. To submit a new model-pack, your PR must include:
...
2. **Cross-Pack Compatibility:** Core changes must be tested against *all* existing model-packs in the repository to prove they haven't introduced model-pack bias.
